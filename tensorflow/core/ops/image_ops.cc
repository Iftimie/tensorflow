...
Status SetOutputToSizedImage(InferenceContext* c, DimensionHandle batch_dim,
  ...
  // Verify shape of size input. size = [crop_height, crop_width]
  ShapeHandle size;
...
// height, width and depth come from the size_tensor.
Status SetOutputToSizedImage3D(InferenceContext* c, DimensionHandle batch_dim,
                             int size_input_idx, DimensionHandle channel_dim) {
  // Verify shape of size input. size = [crop_height, crop_width, crop_depth]
  ShapeHandle size;
  TF_RETURN_IF_ERROR(c->WithRank(c->input(size_input_idx), 1, &size)); //the third input to crop_and_resize is pool size
  DimensionHandle unused;
...
REGISTER_OP("CropAndResize3D")
    ...
    .SetShapeFn([](InferenceContext* c) {
      // Get inputs and validate ranks.
      ShapeHandle input;
      TF_RETURN_IF_ERROR(c->WithRank(c->input(0), 5, &input)); //this one has 5 dimensions [batch, height, width, depth, channels]
      ShapeHandle boxes;
      TF_RETURN_IF_ERROR(c->WithRank(c->input(1), 2, &boxes)); //this one has 2 dimensions [num_rois, 6]
      ShapeHandle box_ind;
      TF_RETURN_IF_ERROR(c->WithRank(c->input(2), 1, &box_ind)); //this one has 1 dimension [num_rois]
      ...
      return SetOutputToSizedImage3D(c, num_boxes_dim, 3 /* size_input_idx */,
                                   c->Dim(input, 4) /*channel_dim*/);
      // [num_boxes, crop_height, crop_width, crop_depth, depth]
    })
    .Doc(R"doc(
Extracts crops from the input volume tensor and bilinearly resizes them (possibly
with aspect ratio change) to a common output size specified by `crop_size`. This
is more general than the `crop_to_bounding_box` op which extracts a fixed size
slice from the input image and does not allow resizing or aspect ratio change.
...
image: A 5-D tensor of shape `[batch, volume_height, volume_width, volume_depth, depth]`.
  Both `volume_height` and `volume_width` need to be positive.
boxes: A 2-D tensor of shape `[num_boxes, 6]`. The `i`-th row of the tensor
  specifies the coordinates of a box in the `box_ind[i]` image and is specified
  in normalized coordinates `[y1, x1, z1, y2, x2, z2]`. A normalized coordinate value of
  `y` is mapped to the image coordinate at `y * (image_height - 1)`, so as the
  `[0, 1]` interval of normalized image height is mapped to
  `[0, image_height - 1]` in image height coordinates. We do allow `y1` > `y2`, in
  which case the sampled crop is an up-down flipped version of the original
...
REGISTER_OP("CropAndResizeGradImage3D")
    .Input("grads: float")
    .Input("boxes: float")
    .Input("box_ind: int32")
    .Input("image_size: int32")
    ...
    .Doc(R"doc(
Computes the gradient of the crop_and_resize3D op wrt the input image tensor.
...
REGISTER_OP("CropAndResizeGradBoxes3D")
    .Input("grads: float")
    .Input("image: T")
    .Input("boxes: float")
    .Input("box_ind: int32")
    ...
    .Doc(R"doc(
Computes the gradient of the crop_and_resize3D op wrt the input boxes tensor.
