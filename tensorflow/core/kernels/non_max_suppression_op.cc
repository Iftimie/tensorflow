...
static inline void ParseAndCheckBoxSizes3D(OpKernelContext* context,
                                         const Tensor& boxes,
                                         const Tensor& scores, int* num_boxes) {
...
// Return true if intersection-over-union overlap between boxes i and j
// is greater than iou_threshold.
static inline bool IOUGreaterThanThreshold3D(
    typename TTypes<float, 2>::ConstTensor boxes, int i, int j,
    float iou_threshold) {
  const float ymin_i = std::min<float>(boxes(i, 0), boxes(i, 3));
  const float xmin_i = std::min<float>(boxes(i, 1), boxes(i, 4));
  const float zmin_i = std::min<float>(boxes(i, 2), boxes(i, 5));
  const float ymax_i = std::max<float>(boxes(i, 0), boxes(i, 3));
  const float xmax_i = std::max<float>(boxes(i, 1), boxes(i, 4));
  const float zmax_i = std::max<float>(boxes(i, 2), boxes(i, 5));

  const float ymin_j = std::min<float>(boxes(j, 0), boxes(j, 3));
  const float xmin_j = std::min<float>(boxes(j, 1), boxes(j, 4));
  const float zmin_j = std::min<float>(boxes(j, 2), boxes(j, 5));
  const float ymax_j = std::max<float>(boxes(j, 0), boxes(j, 3));
  const float xmax_j = std::max<float>(boxes(j, 1), boxes(j, 4));
  const float zmax_j = std::max<float>(boxes(j, 2), boxes(j, 5));
  const float area_i = (ymax_i - ymin_i) * (xmax_i - xmin_i) * (zmax_i - zmin_i);
  const float area_j = (ymax_j - ymin_j) * (xmax_j - xmin_j) * (zmax_j - zmin_j);
  if (area_i <= 0 || area_j <= 0) return 0.0;
  const float intersection_ymin = std::max<float>(ymin_i, ymin_j);
  const float intersection_xmin = std::max<float>(xmin_i, xmin_j);
...
void DoNonMaxSuppressionOp3D(OpKernelContext* context,
                           const Tensor& boxes,
                           const Tensor& scores,
                           const Tensor& max_output_size,
                           const float iou_threshold) {
  ...
  int num_boxes = 0;
  ParseAndCheckBoxSizes3D(context, boxes, scores, &num_boxes);
  if (!context->status().ok()) {
    return;
  }
  ...
  for (int i = 0; i < num_boxes; ++i) {
    ...
    // therefore we iterate through the selected boxes backwards.
    for (int j = num_selected - 1; j >= 0; --j) {
      if (IOUGreaterThanThreshold3D(boxes_data, sorted_indices[i],
                                  sorted_indices[selected_indices[j]],
                                  iou_threshold)) {
        should_select = false;
        break;
...
class NonMaxSuppressionV2Op : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    DoNonMaxSuppressionOp3D(context, boxes, scores, max_output_size,
                          iou_threshold_val);
  }
};

template <typename Device>
class NonMaxSuppressionOp3D : public OpKernel {
 public:
  explicit NonMaxSuppressionOp3D(OpKernelConstruction* context)
      : OpKernel(context) {
  }
...
REGISTER_KERNEL_BUILDER(Name("NonMaxSuppression3D").Device(DEVICE_CPU),
                        NonMaxSuppressionOp3D<CPUDevice>);
